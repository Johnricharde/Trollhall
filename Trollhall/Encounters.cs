...
namespace Trollhall
{
    public class Encounters
    {
        ...
        private void PitfallTrapEncounter()
        {
            ...
            _program.Print(true, "As you walk through a narrow corridor.\nSuddenly, the floor gives way beneath you, revealing a primitive pitfall trap!");
            Program.currentPlayer.health -= damageTaken;
            _program.Print(false, $"You fall into the pit, taking {damageTaken} damage!");
            if (Program.currentPlayer.health > 0)
            {
                _program.Print(true, "Luckily, you manage to climb out of the pit and continue cautiously.");
                RandomEncounter();
            }
            else
            {
        ...
        {
            Console.Clear();
            _program.Print(false, "You enter the ancient Firebrand Brewery,\nonce renowned for its Firebrand Brandy.\nThe air is thick with the scent of fermented ingredients, and\nremnants of barrels and brewing equipment are scattered around.\n");
            _program.Print(false, "What would you like to do?\n");
            _program.Print(false, "1. Investigate the brewing equipment.");
            _program.Print(false, "2. Sample a mysterious brew left on the counter.");
            _program.Print(false, "3. Continue exploring Trollhalls.");
            ...
            {
                case "1":
                    _program.Print(false, "You carefully examine the brewing equipment, discovering a hidden compartment with valuable brewing ingredients.");
                    int rewardCoins = Program.currentPlayer.GetCoins() * 4;
                    Program.currentPlayer.coins += rewardCoins;
                    _program.Print(true, $"You find {rewardCoins} coins worth!");
                    break;

                case "2":
                    _program.Print(false, "Curiosity gets the better of you, and\nyou decide to sample the mysterious brew.\nThe brew has an unexpected kick, you feel a surge of energy!");
                    int rewardXP = Program.currentPlayer.GetXP() * 4;
                    Program.currentPlayer.xp += rewardXP;
                    _program.Print(false, $"You've gained {rewardXP} experience points!");
                ...
                case "3":
                    _program.Print(true, "You decide to continue exploring Trollhalls, leaving the Firebrand Brewery behind.");
                    break;

                default:
                    _program.Print(true, "Invalid choice. Please choose a valid option.");
                    FirebrandBreweryEncounter();
                    break;
            }
            RandomEncounter();
        ...
        {
            Console.Clear();
            _program.Print(false, "You come across a chamber with an intricate dwarven puzzle.\nIt's unclear how to unlock the passage...\n");
            _program.Print(false, " [1] Examine the puzzle and try to solve it.");
            _program.Print(false, " [2] Attempt to brute force the puzzle.\n");
            string playerChoice = Console.ReadLine();
            ...
            switch (playerChoice)
            {
                ...
                case "2":
                    ...
                    if (_rand.Next(0, (Program.currentPlayer.currentClass == Player.PlayerClass.Warrior) ? 4 : 2) == 0)
                    {
                        ...
                        _program.Print(false, "Despite your persistent attempts,\nthe puzzle seems resistant to brute force. \nIt remains unsolved.");
                    break;

                default:
                    _program.Print(true, "Invalid choice. The puzzle remains unsolved.");
                    DwarvenPuzzleEncounter();
                    break;
            }
            void SolvedPuzzle()
        ...
        private void BlacksmithEncounter()
        {
            ...
            switch (upgradeChoice)
            {
                case "1":
                    ...
                    _program.Print(false, $"The blacksmith takes the armor and enhances it.\nYour armor has been upgraded!\nNew armor value: {Program.currentPlayer.armorValue - 1}");
                    break;

                case "2":
                    Program.currentPlayer.weaponValue++;
                    _program.Print(false, $"The blacksmith takes the weapon and enhances it.\nYour weapon has been upgraded!\nNew weapon value: {Program.currentPlayer.weaponValue - 1}");
                    break;

                default:
                    _program.Print(true, "Invalid choice. The blacksmith looks confused.");
                    BlacksmithEncounter();
                    break;
            }
        ...
        private void basicFightEncounter()
        {
            var _combatMechanics = new CombatMechanics();
            Console.Clear();
            _program.Print(true, $"You encounter an enemy!");
