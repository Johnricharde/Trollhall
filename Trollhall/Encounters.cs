...
namespace Trollhall
{
    public class Encounters
    {
        ...
        private static void PitfallTrapEncounter(Program Game)
        {
            ...
            Game.Print(true, "As you walk through a narrow corridor.\nSuddenly, the floor gives way beneath you, revealing a primitive pitfall trap!");
            Program.currentPlayer.health -= damageTaken;
            Game.Print(false, $"You fall into the pit, taking {damageTaken} damage!");
            if (Program.currentPlayer.health > 0)
            {
                Game.Print(true, "Luckily, you manage to climb out of the pit and continue cautiously.");
        ...
        {
            Console.Clear();
            Game.Print(false, "You enter the ancient Firebrand Brewery,\nonce renowned for its Firebrand Brandy.\nThe air is thick with the scent of fermented ingredients, and\nremnants of barrels and brewing equipment are scattered around.\n");
            Game.Print(false, "What would you like to do?\n");
            Game.Print(false, "1. Investigate the brewing equipment.");
            Game.Print(false, "2. Sample a mysterious brew left on the counter.");
            Game.Print(false, "3. Continue exploring Trollhalls.");
            ...
            {
                case "1":
                    Game.Print(false, "You carefully examine the brewing equipment, discovering a hidden compartment with valuable brewing ingredients.");
                    int rewardCoins = Program.currentPlayer.GetCoins() * 4;
                    Program.currentPlayer.coins += rewardCoins;
                    Game.Print(true, $"You find {rewardCoins} coins worth!");
                    break;

                case "2":
                    Game.Print(false, "Curiosity gets the better of you, and\nyou decide to sample the mysterious brew.\nThe brew has an unexpected kick, you feel a surge of energy!");
                    int rewardXP = Program.currentPlayer.GetXP() * 4;
                    Program.currentPlayer.xp += rewardXP;
                    Game.Print(false, $"You've gained {rewardXP} experience points!");
                ...
                case "3":
                    Game.Print(true, "You decide to continue exploring Trollhalls, leaving the Firebrand Brewery behind.");
                    break;

                default:
                    Game.Print(true, "Invalid choice. Please choose a valid option.");
        ...
        {
            Console.Clear();
            Game.Print(false, "You come across a chamber with an intricate dwarven puzzle.\nIt's unclear how to unlock the passage...\n");
            Game.Print(false, " [1] Examine the puzzle and try to solve it.");
            Game.Print(false, " [2] Attempt to brute force the puzzle.\n");
            string playerChoice = Console.ReadLine();

            switch (playerChoice)
            {
                case "1":
                    Game.Print(true, "You carefully examine the dwarven puzzle,\nanalyzing the symbols and their arrangement.\n");
                    if (rand.Next(0, 2) == 0)
                    {
                        Game.Print(true, "After some thought,\nyou successfully decipher the pattern and unlock the passage.");
                        SolvedPuzzle();
                    }
                    else
                        Game.Print(false, "Despite your efforts,\nthe puzzle remains unsolved.\nThe symbols resist your attempts.");
                    break;

                case "2":
                    Game.Print(true, "You decide to take a more direct approach and\nattempt to brute force the puzzle with you strength.\n");
                    if (rand.Next(0, (Program.currentPlayer.currentClass == Player.PlayerClass.Warrior) ? 4 : 2) == 0)
                    {
                        Game.Print(true, "Through trial and error,\nyou manage to brute force the puzzle and unlock the passage.");
                        SolvedPuzzle();
                    }
                    else
                        Game.Print(false, "Despite your persistent attempts,\nthe puzzle seems resistant to brute force. \nIt remains unsolved.");
                    break;

                default:
                    Game.Print(true, "Invalid choice. The puzzle remains unsolved.");
        ...
        {
            Console.Clear();
            Game.Print(true, $"You encounter an enemy!");
